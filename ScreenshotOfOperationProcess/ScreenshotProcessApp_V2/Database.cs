...
namespace ScreenshotProcessApp
{
    ...
    public class Database
    {
        ...
        public void SetFlowStartPage(int flowId, int pageId)
        {
            using (var conn = new SQLiteConnection($"Data Source={_dbPath};Version=3;"))
            {
                conn.Open();
                string sql = "UPDATE ProcessFlow SET StartPageId=@StartPageId WHERE Id=@Id";
                using (var cmd = new SQLiteCommand(sql, conn))
                {
                    cmd.Parameters.AddWithValue("@Id", flowId);
                    cmd.Parameters.AddWithValue("@StartPageId", pageId);
                    cmd.ExecuteNonQuery();
                }
            }
        }

        public PageRegion GetRegionById(int id)
        {
            using (var conn = new SQLiteConnection($"Data Source={_dbPath};Version=3;"))
            {
                conn.Open();
                string sql = "SELECT Id, PageId, X, Y, Width, Height, Remark, TargetPageId FROM PageRegion WHERE Id=@Id";
                using (var cmd = new SQLiteCommand(sql, conn))
                {
                    cmd.Parameters.AddWithValue("@Id", id);
                    using (var reader = cmd.ExecuteReader())
        ...
        public PageAnnotation GetAnnotationById(int id)
        {
            using (var conn = new SQLiteConnection($"Data Source={_dbPath};Version=3;"))
            {
                conn.Open();
                string sql = "SELECT Id, PageId, TextX, TextY, TextWidth, TextHeight, Text, ArrowEndX, ArrowEndY FROM PageAnnotation WHERE Id=@Id";
                using (var cmd = new SQLiteCommand(sql, conn))
                {
                    cmd.Parameters.AddWithValue("@Id", id);
                    using (var reader = cmd.ExecuteReader())
