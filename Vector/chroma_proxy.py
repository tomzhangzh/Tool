...
async def query_documents(collection_name: str, request: QueryRequest):
    try:
        ...
        if request.site_name:
            where_filter = {"site_name": request.site_name}
...
@app.get("/api/collection/{collection_name}/sites")
async def get_sites(collection_name: str):
    try:
        collection = client.get_collection(name=collection_name)
        ...
        if results and 'metadatas' in results:
            for metadata in results['metadatas']:
                if metadata and 'site_name' in metadata:
                    sites.add(metadata['site_name'])
