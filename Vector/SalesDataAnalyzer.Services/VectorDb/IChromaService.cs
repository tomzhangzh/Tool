...
public interface IChromaService
{
    ...
    Task<List<(string Document, float Score, Dictionary<string, string> Metadata)>> QuerySimilarDocumentsAsync(string collectionName, string query, string? siteName = null, int nResults = 5);
    Task DeleteDocumentAsync(string collectionName, string documentId);
    Task<int> GetCollectionCountAsync(string collectionName);
    Task<List<string>> GetAllSitesAsync(string collectionName);
}