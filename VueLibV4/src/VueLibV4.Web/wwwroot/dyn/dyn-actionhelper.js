/* ============================================================
 * VueLibV4  dyn-actionhelper.js
 * 以 V1 完整版为基底（事件委托 ACTION_EVENTS 含 init、dyn-{event}-{action}、
 * $ctx 上下文、内置动作 postback/postdata/confirm-postdata/reload/open/close/
 * updateel/evaljs/load/setVueModel/showmessage/redirect/chain/openwindow/...、
 * setVueModel 用 lodash _.set + el.__dynProxy、evaljs 事件+init 双用）。
 * 叠加 V4：从 /api/platform/dynactionhelper/all 动态注册入库动作，
 * 并向 V3 运行时导出 global.DynActionHelper.run(code,args,ctx)。
 * ============================================================ */
(function (global) {
    'use strict';
    ...
    var DYN_CFG = {
        PREFIX: '[dyn]',
        ACTION_API_BASE: '/api/platform/dynactionhelper',
    ...
    // 初始绑定（V4 增量桥在文件末尾从 /api/platform/dynactionhelper/all 加载入库动作）
    try { autoBindActions(); } catch (e) { }
    // V1 旧 /api/dynactionhelper 在 V4 不存在，由末尾 V4 增量桥负责加载，故此处不再调用
...
/* ============================================================
 * V4 增量：DB 入库动作动态注册 + V3 运行时桥 global.DynActionHelper
 * - 从 /api/platform/dynactionhelper/all 拉取入库动作（PascalCase 信封）
 * - run(code,args,ctx)：V1 内置动作优先（dyn.resolveAction），否则执行入库动作
 * - 不改动 V1 执行引擎与内置动作语义
 * ============================================================ */
(function (global) {
  'use strict';
  ...
  var api = {
    msg: function (m, t) {
      ...
      if (ElMessage) ElMessage({ message: m || '', type: t || 'success' }); else window.alert(m);
    },
    alert: function (m) {
      var mb = global.ElementPlus && global.ElementPlus.ElMessageBox;
      if (mb) mb.alert(m || '提示', '提示', { confirmButtonText: '确定' }); else window.alert(m);
    },
    confirm: function (m, title) {
      var mb = global.ElementPlus && global.ElementPlus.ElMessageBox;
      if (mb) return mb.confirm(m || '确定执行吗？', title || '提示', { type: 'warning', confirmButtonText: '确定', cancelButtonText: '取消' }).then(function(){return true;}).catch(function(){return false;});
      return Promise.resolve(window.confirm(m || '确定？'));
    },
    openUrl: function (u, t) { window.open(u, t || '_blank'); },
    refresh: function () { window.location.reload(); },
    http: function (u, method, body) {
      return fetch(u, { method: method || 'GET', headers: { 'Content-Type': 'application/json' }, body: body ? JSON.stringify(body) : undefined }).then(function (r) { return r.json(); });
    }
  };

  function register(a) { if (a && a.Code) DB_ACTIONS[a.Code] = a; }
  function registerAll(list) { (list || []).forEach(register); }
  function lookup(code) {
    if (DB_ACTIONS[code]) return DB_ACTIONS[code];
    var keys = Object.keys(DB_ACTIONS);
    for (var i = 0; i < keys.length; i++) if (keys[i].toLowerCase() === String(code).toLowerCase()) return DB_ACTIONS[keys[i]];
    return null;
  }
  function loadAll() {
    if (loaded) return Promise.resolve(DB_ACTIONS);
    loaded = true;
    return fetch(API_BASE + '/all').then(function (r) { return r.json(); })
      .then(function (res) { registerAll(res && res.data ? res.data : []); return DB_ACTIONS; })
      .catch(function (e) { console.error('[V4 action] 加载入库动作失败', e); return DB_ACTIONS; });
  }

  function runBuiltin(code, args, ctx) {
    var dyn = global.dynCore || global.dyn;
    if (!dyn || !dyn.resolveAction) return null;
    var fn = dyn.resolveAction(code);
    if (!fn) return null;
    var el = (args && args.el) || (ctx && ctx.el) || document.body;
    var opts = (args && (args.options || (typeof args === 'object' && !args.Code ? args : {}))) || {};
    var v1ctx = {
      element: el, el: el,
      event: 'program', $event: (args && args.$event) || null, targetInfo: null,
      action: code, options: opts, params: (args && args.params) || {},
      model: (ctx && ctx.pageCtx && ctx.pageCtx.pageData) || null,
      vm: null, url: (args && args.url) || ''
    };
    return Promise.resolve(fn(v1ctx));
  }

  function fillTpl(tpl, args) {
    return String(tpl || '').replace(/\{(\w+)\}/g, function (_, k) { return args && args[k] !== undefined ? args[k] : ''; });
  }
  ...
  function runUrl(def, args) {
    var u = fillTpl(def.Script, args);
    var t = (args && args.target) || 'self';
    if (u) window.open(u, t === '_blank' ? '_blank' : '_self');
    return Promise.resolve(u);
  }
  function runApi(def, args) {
    var u = fillTpl(def.Script, args);
    return api.http(u, (args && args.method) || 'GET', args && args.body).then(function (r) { return r && r.data; });
  }
  function runChain(def, args, ctx) {
    ...
    return steps.reduce(function (p, step) {
      return p.then(function (last) {
        if (step.when === 'result' && !last) return prev;
  ...
  function run(code, args, ctx) {
    args = args || {}; ctx = ctx || {};
    var builtin = runBuiltin(code, args, ctx);
    if (builtin) return builtin;
    var def = lookup(code);
    if (!def) { console.warn('[V4 action] 动作不存在: ' + code); return Promise.resolve(null); }
    try {
      switch (def.ActionType) {
        case 'url': return runUrl(def, args);
        case 'api': return runApi(def, args);
        case 'chain': return runChain(def, args, ctx);
        case 'script':
        default: return runScript(def, args, ctx);
      }
    } catch (e) { console.error('[V4 action] 执行异常', def.Code, e); return Promise.reject(e); }
  }

  function init() {
    loadAll().then(function () {
      var dyn = global.dynCore || global.dyn;
      if (dyn && dyn._initActions) { try { dyn._initActions(document.body); } catch (e) { } }
    });
  }
  ...
  global.DynActionHelper = {
    register: register, registerAll: registerAll, loadAll: loadAll, get: lookup,
    run: run, runChain: runChain, actions: DB_ACTIONS, api: api
  };
})(window);
