/* ============================================================
 * VueLibV4  dyn-core.js
 * 以 V1 完整版为基底（mountCore / init / reload / postback / open / close /
 * findAncestor / closestDynInit / getVueModel / setPathVal / __expr / eventBus /
 * runJsonActions），merge V3 增量（DynRender 递归渲染 + normalize/clean/isEmpty
 * 空值处理 + getPath/setPath/evalValue/mount）。
 * 导出 global.dynCore / global.dyn（V1 执行引擎）与 global.DynCore（V3 渲染核）。
 * 依赖：Vue3 UMD、jQuery、lodash（可选）、ElementPlus（可选）
 * ============================================================ */
(function (global) {
    'use strict';
...
/* ============================================================
 * V3 增量：DynRender 递归渲染核心（设计器/运行时共用）
 * 导出 global.DynCore = { DynRender, getPath, setPath, evalValue, normalize, clean, mount }
 * ============================================================ */
(function (global) {
  'use strict';
  ...
  var DynRender = {
    ...
    render: function () {
      ...
      /* ---- V1 组件契约：jsonconfig 整树（DynCrudPage、DynTable、Grid3、Card、Row、Col、DivContainer 等 Razor View 组件） ----
       * 以 'El' 开头的为 ElementPlus 全局组件（直接接收 props，不传 jsonconfig）；
       * 其余 V1 组件（DynCrudPage/DynTable/Grid3/Card/ForEach/Fragment/Button/Tag...）在
       * _ElementLayout.setup 中读取 props.jsonconfig 填充默认值并派生 jc/childrenctrls。
       * 若不传 jsonconfig，DynCrudPage 等组件的 jc 为空 → 逻辑失效/递归渲染栈溢出。
       */
      ...
      if (this.pageCtx && this.pageCtx.designMode) {
        ...
        if (this.pageCtx.onSelect) {
          on.onClickCapture = function (e) {
            // capture 阶段监听：内部子组件（如 el-radio）stopPropagation 也不会阻断选中
      ...
      /* ---- 无 label：display:contents 包裹层承载 uid/拖拽/选中事件 ----
       * 不产生布局盒子（display:contents），grid/flex 布局中组件本体仍是直接子项，
       * col-span-2 等 class 透传到组件根元素（ElementPlus class 透传）→ 布局不受影响；
       * 事件落在包裹层（组件区域任意冒泡均可达），选中/拖拽不会因 ElInput 等
       * 组件把 attrs 透传到内层 input 而失效。
       */
      var designOn = {};
      if (this.pageCtx && this.pageCtx.designMode) {
        ['onDragstart', 'onClickCapture', 'onMouseover', 'onDragover', 'onDrop', 'onDragleave'].forEach(function (k) {
          if (on[k]) { designOn[k] = on[k]; delete on[k]; }
        });
      }
      var compAttrs = {};
      if (outer.class) compAttrs.class = outer.class;
      if (outer.style) compAttrs.style = outer.style;
      var wrapProps = { style: { display: 'contents' } };
      if (outer['data-dyn-uid']) wrapProps['data-dyn-uid'] = outer['data-dyn-uid'];
      if (outer.draggable) wrapProps.draggable = true;
  ...
  /** 便捷挂载：先把 DB/View 组件清单注册完成，再把配置渲染到指定元素（V1/V4 组件双源一致） */
  function mount(cfg, el, overrides) {
    ...
    if (global.DynCom) {
      ...
      // 关键：注册 DB/View 组件清单（DynCrudPage/DynTable/Grid3/El* 等），
      // 否则 DynRender resolveComponent 失败显示"未知组件"
