...
namespace TUI.WebPortal.Controllers
{
    ...
    public class BaseController : Controller
    {
        ...
        {
            List<string> scripts = HttpContext.Session.GetValue<List<string>>("CustomScripts");
            if (scripts == null)
            {
                scripts = new List<string>();
            }
            scripts.Add(script.Script);
            HttpContext.Session.SetValue("CustomScripts", scripts);
